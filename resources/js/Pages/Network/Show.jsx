...
const getDistinctionStyles = (distinction) => {
    if (!distinction) return "bg-gray-100 text-gray-600 border-gray-300";
    switch (distinction) {
        case "Gold":
...
const DownlineTree = ({ user, level = 0 }) => {
    if (!user || typeof user !== "object") {
        ...
        return (
            <span className="text-red-500">Données utilisateur invalides</span>
        );
    }
    ...
    return (
        <div className="ml-4 border-l pl-4">
            ...
            {user.downlines &&
            Array.isArray(user.downlines) &&
            user.downlines.length > 0 ? (
                user.downlines.map((downline) => (
                    <DownlineTree
                        key={
                            downline.id ||
                            `${level}-${downline.username || "unknown"}`
                        }
                        user={downline}
                        level={level + 1}
                    />
                                        ...
                                        <tbody>
                                            {sponsorGains.data.map((gain) => (
                                                ...
                                                >
                                                    ...
                                                    <td className="py-2 px-4">
                                                        {gain.buyer ? gain.buyer.username : "Utilisateur inconnu"}
                                                    </td>
                                                    <td className="py-2 px-4">
